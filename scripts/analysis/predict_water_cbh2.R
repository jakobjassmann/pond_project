...
raster_files_norm <- list.files("data/drone_time_series/cbh/cbh_norm2/",
                           full.names = T)
...
plot_grid(
  ...
  labels = paste0(letters[1:6], ")")) %>%
    save_plot("figures/cbh/sepparation_all2.png",
    .,
    nrow = 2,
    ncol = 2,
    base_asp = 1.2,
...
plot_sep_band <- function(band = "R"){
  ...
  if(band %in% c("R", "G", "B")) cbh_training$band_to_plot <- cbh_training$band_to_plot / 65535
  save_plot(paste0("figures/cbh/sepparation_", band, "_year2.png"),
            ggplot(cbh_training) +
...
# Save the model
save(rf_fit, file = "data/models/cbh_rf_with2016_2.Rda")
...
dir.create("data/drone_time_series/cbh/cbh_preds2/")
preds_rasters <- map(years, function(year_interest){
  ...
  writeRaster(preds, 
              filename =  gsub(".*/(.*_[0-9]{4}).*", 
                               "data/drone_time_series/cbh/cbh_preds2/\\1_preds.tif", 
