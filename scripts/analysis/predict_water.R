...
raster_files_norm <- list.files("data/drone_time_series/cbh_timeseries/norm/",
                           full.names = T)
...
# Set years on record
years <- c("2014_byte", "2016", "2017", "2018", "2019", "2019_b", "2020", "2021")
...
get_training_vals <- function(year_interest){
  ...
  # Load raster
  norm_raster <- rast(raster_files_norm[grepl(paste0(".*", year_interest, "\\.tif"), raster_files_norm)])
  # Subset polys for year
  polys <- cbh_polys[pull(cbh_polys, paste0(substr(year_interest, 1, 4))) == 1, ]
