# Quick script to visualise preddictions
...
plot_preds <- function(preds_file){
  ...
  norm_file <- norm_files %>% .[grepl(site_interest,.)] %>% .[grepl(year_interest,.)]
  plot_grid(
    ggplot() +
...
pblapply(preds_cbh, plot_preds, cl = 12) %>% 
  plot_grid(plotlist = .) %>%
  save_plot(filename = "figures/cbh_preds.png",
            bg = "white",
            base_height = 10)
pblapply(preds_tlb, plot_preds, cl = 12) %>% 
  plot_grid(plotlist = .) %>%
  save_plot(filename = "figures/tlb_preds.png",
            bg = "white",
            base_height = 10)
pblapply(preds_rdg, plot_preds, cl = 12) %>% 
  plot_grid(plotlist = .) %>%
  save_plot(filename = "figures/rdg_preds.png",
            bg = "white",
            base_height = 10)
