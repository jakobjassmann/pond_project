...
# Load list of images
raster_files <- list.files(pattern = "tif", "data/drone_time_series/dtlb/dtlb_original/",
                           full.names = T)

# Load tiepoints
tie_points <- read_sf("data/tiepoints/dtlb_tiepoints.shp") %>%
...
# make output dir
dir.create("data/drone_time_series/dtlb/dtlb_warped")
...
warp_with_gcps <- function(year_interest){
  ...
  # Set tempfile name
  temp_file <- gsub("(data/drone_time_series/)(.*)/.*",
  ...
  out_file <- gsub("(data/drone_time_series/dtlb/)(.*)/.*",
...
tie_points <- read_sf("data/tiepoints/dtlb_tiepoints.shp")
tie_points_post_warp <- read_sf("data/drone_time_series/dtlb/dtlb_warped/tiepoints_post_warp.shp") %>%
  st_transform(crs = st_crs(tie_points))
...
tie_points_post_warp %>% 
  ...
  unlist() %>%
  mean()
# As expected it got worse! Use the non-warped data. 

# Copy files to a new folder for processing
dir.create("data/drone_time_series/dtlb/dtlb_ready/")
file.copy("data/drone_time_series/dtlb/dtlb_original/dtlb_2020.tif",
 "data/drone_time_series/dtlb/dtlb_ready/dtlb_2020.tif")
file.copy("data/drone_time_series/dtlb/dtlb_original/dtlb_2021.tif",
 "data/drone_time_series/dtlb/dtlb_ready/dtlb_2021.tif")
ready_files <- list.files("data/drone_time_series/dtlb/dtlb_warped/")
map(ready_files, function(x) {
  file.copy(paste0("data/drone_time_series/dtlb/dtlb_warped/", x),
            paste0("data/drone_time_series/dtlb/dtlb_ready/", x))
} )