...
# Load training polys
thaw_lakebed_polys <- read_sf("data/training/dtlb_polys.shp") %>%
st_transform(crs = 32655)

# Load list of raster files 
raster_files_norm <- list.files("data/drone_time_series/dtlb/dtlb_norm/",
                           full.names = T)
raster_files_focal_mean <- list.files("data/drone_time_series/dtlb/dtlb_focal_mean/",
                                full.names = T)
raster_files_focal_sd <- list.files("data/drone_time_series/dtlb/dtlb_focal_sd/",
                                      full.names = T)
...
#             data = select(training, -id))
rf_fit <- ranger(class ~ ., data = select(training, -id))

# Validate on test set
test_preds <- predict(rf_fit, data = validation,
                      type = "response")
confusionMatrix(data = test_preds$predictions, validation$class)

# Save the model
save(rf_fit, file = "data/models/dtlb_ranger.Rda")

# Load model if needed
#load("data/models/dtlb_ranger.Rda")

# Let's see how that looks like in space
# Predictions only work with raster
library(raster)
dir.create("data/drone_time_series/dtlb/dtlb_preds/")
preds_rasters <- map(years, function(year_interest){
  cat("Running projections for", year_interest, ":\n")
  cat("Preparing data...\n")
  norm_raster <- brick(raster_files_norm[grepl(year_interest, raster_files_norm)])
  mean_raster <- brick(raster_files_focal_mean[grepl(year_interest, raster_files_focal_mean)])
  sd_raster <- brick(raster_files_focal_sd[grepl(year_interest, raster_files_focal_sd)])
  ...
  predictors <- stack(norm_raster[[-4]], mean_raster[[-4]], sd_raster[[-4]])
  cat("Predicting values...\n")er
  preds <-  predict(predictors, rf_fit)
  cat("Writing raster...\n")
  writeRaster(preds, 
              filename =  gsub(".*/(.*_[0-9]{4}).*", 
                               "data/drone_time_series/dtlb/dtlb_preds/\\1_preds.tif", 
