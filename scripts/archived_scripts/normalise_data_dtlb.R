...
raster_files <- list.files("data/drone_time_series/dtlb/dtlb_ready/",
                           full.names = T)
...
norm_raster <- function(raster_file){
  ...
  cat("Normalizing Band 3 (B)B:\n")
  raster_object[[3]] <- (raster_object[[3]] - bands_min[3,]) / (bands_max[3,] - bands_min[3,])
  cat("Cropping to aoi extent:\n")
  raster_object <- crop(raster_object, vect("data/study_aois/dtlb.shp"))
  cat("Writing raster...\n")
  writeRaster(raster_object, 
              gsub(".*/(.*_[0-9]{4}).*", 
                   "data/drone_time_series/dtlb/dtlb_norm/\\1_norm.tif", 
                   raster_file),
              overwrite = T)
  cat("Done.\n")
  return(NULL)
