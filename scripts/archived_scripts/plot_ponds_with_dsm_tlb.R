...
rast_files <- tlb_norm
# Load dsm 
tlb_dsm <- list.files("data/drone_time_series/tlb_timeseries/dsm/",
                           pattern = "tif",
                           full.names = T) 
dsm_files <- tlb_dsm

# Load predictions 
tlb_preds <- list.files("data/drone_time_series/tlb_timeseries/preds_filtered_overlap//",
                      pattern = "tif",
                      full.names = T) 
preds_files <- tlb_preds

# Load ponds
tlb_ponds <- read_sf("data/pond_polys/pond_polys_filtered_size_overlap.gpkg") %>%
  filter(site == "tlb")

ggplot() + geom_sf(data = tlb_ponds %>% filter(year == "2014") %>%
  mutate(id = 1:nrow(.)), mapping = aes(fill = as.factor(id))) +
  geom_sf_text(data = tlb_ponds %>% filter(year == "2014") %>%
                  mutate(id = 1:nrow(.)), mapping = aes(label = (id))) +
  theme_nothing() +
  theme(legend.position = "none")

pond <- tlb_ponds %>% filter(year == "2014") %>%
  mutate(id = 1:nrow(.)) %>%
  filter(id == 66)
plot(st_geometry(pond))
plot(st_geometry(st_buffer(pond, 5)), add = T)
pond_2014_rgb <- pond
pond_2014_dsm <- pond

prefix <- "tlb"
# Set pond boundaries
pond_bounds <- ext(st_buffer(pond, 5))
...
plot_pond_ts <- function(pond_bounds, rast_files, preds_files, dsm_files, prefix,
  ...
  rgb_plots <- map(rast_files, function(x){
    ...
    #pond_bounds <- ext(st_buffer(pond, 15))
    
    # crop
    x_crop <- crop(x, pond_bounds)
    
    # Get year
    year <- gsub(".*([0-9]{4}_?[abc]?).*", "\\1", sources(x))
  ...
  dsm_plots <- map2(preds_files, dsm_files, function(preds, dsm){
    ...
    dsm_crop <- mask(dsm_crop,classify(preds_crop, matrix(c(1, NA,  NA, 1), byrow = T, ncol = 2)))
    ...
    dsm_plot <- ggplot() +
      ...
      geom_spatraster(data = preds_crop,
                      ...
                      fill = water_colour, na.rm = FALSE) +
      scale_alpha_continuous(range = c(0, 1), na.value = 0) +
