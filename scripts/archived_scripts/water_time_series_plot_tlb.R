# Visualisation of the surface water area cover over time for tlb2
...
area_water_per_year <- map(tlb_preds, function(x){
  rast_obj <- rast(x)
  data.frame(
...
write_csv(area_water_per_year,
          "data/drone_time_series/tlb_timeseries/area_water_by_year.csv")
...
time_series <- ggplot(area_water_per_year) +
  ...
  theme_cowplot(20)
save_plot("figures/tlb/time_series_open_water.png",
          time_series, 
          bg = "white",
          base_asp = 1.8)
...
time_series <- ggplot(area_water_per_year) +
  ...
  theme_cowplot(20)
save_plot("figures/tlb/time_series_open_water_prop.png",
          time_series, 
          bg = "white",
          base_asp = 1.8)
...
time_of_observation <- ggplot(time_of_year) +
  ...
  coord_flip() +
  theme_cowplot()
save_plot("figures/tlb/day_of_drone_survey.png",
