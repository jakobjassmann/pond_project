...
# Set target image
target_image <- "C:/Scratch/pond_project/cbh/cbh_2016/cbh_2016/img/IMG_0700.JPG"

# Read in images 
image_list <- data.frame(
    file = list.files("C:/Scratch/pond_project/cbh/cbh_2016/cbh_2016/img",
    full.names = T)
)
...
copy_tags <- function(src){
    shell(paste0("C:/Scratch/pond_project/exiftool/exiftool.exe",
     " -tagsfromfile ", src, 
     " -all:all ", gsub("(.*)(\\..*)", "\\1_corrected\\2", src)))
}
